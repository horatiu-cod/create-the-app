...
def run_ollama_document_converter_cloud(input_doc_path: Path, model_name: str = "gemma3:27b-cloud") -> tuple[str, bool]:
    ...
    vlm_options = ApiVlmOptions(
        ...
        prompt="Convert this PDF document to markdown format. Do not generate any explanation. Do not add any extra text",  # Added
        params={
            "model": model_name,
            "temperature": 0.0,
            "max_tokens": 8192,
...
def run_ollama_document_converter_router(input_doc_path: Path, model_name: str = "google/gemma-3-27b-it:free") -> tuple[str, bool]:
    # Load environment variables
    # from dotenv import load_dotenv

    load_dotenv()
    base_url = os.environ.get("OPENROUTER_BASE_URL", "")
    REMOTE_OLLAMA_URL = f"{base_url}/chat/completions"
    print(base_url)
    print(REMOTE_OLLAMA_URL)
    try:
        
        vlm_options = ApiVlmOptions(
            url=REMOTE_OLLAMA_URL,
            prompt="Convert this PDF document to markdown format. Do not generate any explanation.",  # Added
            params={
                "model": model_name,
                "temperature": 0.0,
                "max_tokens": 8192,
        ...
        logger.error(f"Error creating VlmPipelineOptions: {e}")
        return None, False
